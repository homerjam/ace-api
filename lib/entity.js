...
class Entity {
  ...
  entitiesById (ids, parents = null, role = 'guest') {
    return new Promise((resolve, reject) => {
      ...
      Db.connect(this.config).view('entity', parents ? 'byIdExtended' : 'byId', {
        ...
        .then((result) => {
          const docs = Entity._appendParents(result, parents, role)
  ...
  async _getDocMap (docs, children, parents, role, docMap = {}, childDepth = 0) {
    ...
    let _docs = await this.entitiesById(ids, parents, role);
