...
class Entity {
  ...
  static _childDepthLimit (children) {
    ...
    if (_.isArray(children) && _.isArray(children[0])) {
  ...
  async _getDocMap (docs, children, parents, role, docMap = {}, childDepth = 0) {
    ...
    docs.forEach((rowOrDoc) => {
      ...
      if (children && _.size(doc.fields)) {
        if (_.isArray(children)) {
          const queries = _.isArray(children[0]) ? children[childDepth] : children;
          ...
          queries.forEach((query) => {
            ...
            const modifier = /\]:/.test(query) ? `:${query.split(/\]:/).slice(-1)[0]}` : '';
            ...
            ids = ids.concat(_.flatten(result).map(obj => obj.id));
          });
    ...
    const childDepthLimit = Entity._childDepthLimit(children);

    childDepth += 1;

    if (!children || (childDepth >= childDepthLimit)) {
      return docMap;
    }
  ...
  static _mergeDocs(docs, docMap, children, childDepth = 0) {
    ...
    docs = docs.map((rowOrDoc) => {
      ...
      if (children && doc.fields) {
        ...
        let queryFields;
        if (_.isArray(children)) {
          const queries = _.isArray(children[0]) ? children[childDepth - 1] : children;
          queryFields = queries.map(query => query.split(/\[|\]/)[0]);
        }
        ...
        doc.fields = _.mapValues(doc.fields, (field, fieldSlug) => {
          if (_.isArray(field.value)) {
            ...
            if (queryFields.indexOf(fieldSlug) > -1) {
              field.value = field.value.filter(obj => obj && docMap[obj.id]);
            }
            ...
            // TODO: filter/pick entity fields
          }
