...
class Entity {
  ...
  static mergeEntities (docs, entities, isRow) {
    let entitiesMap = {};

    entities.forEach((doc) => {
      // Prevent circular reference error
      entitiesMap[doc._id] = _.clone(doc);
    });

    docs = docs.map((doc) => {
      const entity = entitiesMap[doc.id || doc._id];
  ...
  static flattenValues (docs) {
    return docs.map((doc) => {
      ...
      doc.fields = _.mapValues(doc.fields, (field) => {
        if (/entity/.test(field.fieldType) && _.isArray(field.value)) { // entity / entityTile / entityGrid
  ...
  _extendDocs (docs, children, parents, role, childDepth = 1) {
    return new Promise((resolve, reject) => {
      ...
      // get entity(s) and merge children
      this.entitiesById(ids, children, parents, role)
        .then((_docs) => {
          // filter entityField(s)
          _docs = Entity.filterEntityFieldsByRole(_docs, role);

          // map/merge entity(s)
          docs = Entity.mergeEntities(docs, _docs);
          ...
          docs = docs.map((doc) => {
            ...
            doc.fields = _.mapValues(doc.fields, (field) => {
              if (/entity/.test(field.fieldType)) { // entity / entityTile / entityGrid
      ...
      Db.connect(this.config).viewAsync('entity', 'byId', query)
        .then((result) => {
          ...
          this._extendDocs(result.rows, children, parents, role)
            .then((docs) => {
              result.rows = Entity.mergeEntities(result.rows, docs);
    ...
    return new Promise((resolve, reject) => {
      ...
      Db.connect(this.config).searchAsync('entity', query.index || 'all', query)
        .then((result) => {
            ...
            result.groups = result.groups.map((group) => {
              promises.push(new Promise((resolve, reject) => {
                ...
                this._extendDocs(group.rows, children, parents, role)
                  .then((docs) => {
                    group.rows = Entity.mergeEntities(group.rows, docs, true);
                    resolve();
                  }, reject);
              }));
              return group;
          ...
          this._extendDocs(result.rows, children, parents, role)
            .then((docs) => {
              result.rows = Entity.mergeEntities(result.rows, docs, true);
  ...
  entityFind (query, children = null, parents = null, role = 'guest') {
    return new Promise((resolve, reject) => {
      Db.connect(this.config).findAsync(query)
        .then((result) => {
          ...
          this._extendDocs(result.docs, children, parents, role)
            .then((docs) => {
              result.docs = Entity.mergeEntities(result.docs, docs);
