  ...
  async _getDocMap (docs, children, parents, role, docMap = {}, childDepth = 0) {
    ...
    docs.forEach((rowOrDoc) => {
      ...
      if (children && doc.fields && _.size(doc.fields)) {
        if (_.isArray(children)) {
          const queries = children[childDepth].split(';');

          queries.forEach((query) => {
            ids = ids.concat(_.flatten(Entity._query(doc, query).value).map(obj => obj.id));
          });
    ...
    docMap = await this._getDocMap(_docs, children, parents, role, docMap, childDepth + 1);
  ...
  static _mergeDocs(docs, docMap, children, childDepth = 0) {
    docs = docs.map((rowOrDoc) => {
      ...
      if (children && doc.fields && _.size(doc.fields)) {
        ...
        if (_.isArray(children)) {
          children[childDepth].split(';').forEach((query) => {
            query = query.trim();
