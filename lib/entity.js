...
class Entity {
  ...
  async _getDocMap(rowsOrDocs, docMap = {}, options = {}) {
    ...
    const docs = (await this._entitiesById(ids, options)).rows.map(row => row.doc);
    const childDocs = (await this._entitiesById(childIds, { ...options, parents: false })).rows.map(row => row.doc);
