...
class Entity {
  ...
  async _getDocMap (docs, children, parents, role) {
    const docMap = {};
    let childDepth = 0;

    const getDocs = async (_docs) => {
      if ((!parents && !children)) {
        return;
      }
  ...
  static _mergeDocs(docs, docMap, children, childDepth = 0) {
    ...
    docs = docs.map((rowOrDoc) => {
      ...
      const doc = _.clone(docMap[rowOrDoc.id || rowOrDoc._id] || (isRow ? rowOrDoc.doc : rowOrDoc));

      if (children && doc.fields) {
        doc.fields = _.mapValues(doc.fields, (field) => {
          if (_.isArray(field.value)) {
            field.value = field.value.map(obj => (obj.id && docMap[obj.id] ? _.merge({}, obj, docMap[obj.id] || {}) : obj));
  ...
  async _extendDocs (docs, children, parents, role) {
    const docMap = await this._getDocMap(docs, children, parents, role);
