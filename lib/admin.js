  ...
  getUser(userId) {
    return new Promise((resolve, reject) => {
      ...
      Db.connect(this.config).getAsync(userId)
        .then((user) => {
          if (isSuperUser || this.config.environment !== 'production') {
            user = {
              _id: userId,
              active: true,
              email: userId,
              firstName: user.firstName || this.config.dev.role,
    ...
    return new Promise((resolve, reject) => {
      Db.connect(this.config).viewAsync('admin', 'fieldByKey', {
        ...
        .then((body) => {
          ...
          Db.connect(this.config).indexAsync(activeIndex)
            .then(() => {
              Db.connect(this.config).indexAsync(publishedIndex)
                .then(resolve);
            });
        }, reject);
    });
