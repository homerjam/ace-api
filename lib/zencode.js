...
class Zencode {
  ...
  _updateFile(fileId, zencoderJob) {
    this.db().viewAsync('entity', 'byFile', {
      keys: [fileId],
      include_docs: true,
    })
      .then((body) => {
        ...
        Helpers.createOrUpdate(this.db, entity);
      });

    this.db().getAsync(fileId)
      .then((file) => {
        file.metadata.zencoder = zencoderJob;

        Helpers.createOrUpdate(this.db, file);
      });
  }
      ...
      this.s3.upload(fileName, options.uploadOptions)
        .then((s3Upload) => {
          ...
          const _outputs = options.outputs.map((output, index) => {
            const baseUrl = `${options.metadata.bucket + (slug !== '' ? '/' + slug : '')}/${options.metadata.base}`;
