...
class StripeClass {
    ...
    return new Promise((resolve, reject) => {
      this.getSettings()
        .then((settings) => {
          ...
          if (order.subscribe && settings.createsend && settings.createsend.checkoutSubscriberListId) {
            this.email.subscribe(order.customerDetails, 'createsend', settings.createsend.checkoutSubscriberListId)
              .then((result) => {
                console.log(result);
              }, (error) => {
                console.error(error);
  ...
  sendReceipt(settings, customer, order) {
    return new Promise((resolve, reject) => {
      ...
      this.email.sendEmail(emailOptions, `${this.config.slug}/order-receipt`, templateData).then(resolve, reject);
    });
  }
  ...
  sendNotification(settings, customer, order) {
    return new Promise((resolve, reject) => {
      ...
      this.email.sendEmail(emailOptions, `${this.config.slug}/order-notification`, templateData).then(resolve, reject);
    });
  }
