...
class Stripe {
  ...
  getSettings() {
    return new Promise((resolve, reject) => {
      ...
      const ecommerce = new Ecommerce(this.db, this.config);

      ecommerce.settings()
        .then((settings) => {
          try {
            settings.stripeAccountId = settings.stripe.stripe_user_id;
          } catch (error) {
            reject({
              statusCode: 404,
        ...
        .then((settings) => {
          this.stripe.refunds.create({
            ...
            .then((refund) => {
              ...
              Helpers.createOrUpdate(this.db, order)
                .then(resolve, reject);
            }, reject);
        }, reject);
    });
  ...
  findOrCreateCustomer(email, order) {
    return new Promise((resolve, reject) => {
      this.db().viewAsync('ecommerce', 'customerByEmail', {
        keys: [email],
        include_docs: true,
      })
        .then((body) => {
          if (body.rows.length) {
            ...
            this.db().insertAsync(newCustomer)
              .then((body) => {
