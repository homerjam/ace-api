  ...
  getTemplate(template, templateData, options = {}) {
    return new Promise((resolve, reject) => {
      ...
      const style = sass.renderSync({
        ...
        sourceMapContents: options.previewOnly,
        sourceMapEmbed: options.previewOnly,
      }).css.toString().replace(/"/g, '\'');

      const emailTemplate = new EmailTemplate(templatePath, {
        disableJuice: options.previewOnly,
        juiceOptions: {
      ...
      templateData.previewOnly = options.previewOnly;
      templateData.baseUrl = this.config.baseUrl;
      templateData.moment = moment;
      templateData.countries = countries;

      emailTemplate.render(templateData, (error, result) => {
        if (error) {
          reject(error);
          return;
        }
