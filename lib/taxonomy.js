...
class Taxonomy {
  ...
  //     Helpers.createOrUpdate(this.db, taxonomy)
  //       .then(resolve, reject);
  //   });
  // }
  ...
  read(taxonomySlug) {
    return new Promise((resolve, reject) => {
      this.db().viewAsync('taxonomy', 'byKey', {
        key: taxonomySlug,
        include_docs: true,
      })
        .then((body) => {
          if (!body.rows.length) {
            reject(`Taxonomy not found: ${taxonomySlug}`);
            return;
          }
  ...
  updateTerm(updatedTerm) {
    return new Promise((resolve, reject) => {
      this.db().viewAsync('entity', 'byTaxonomyTerm', {
        keys: [updatedTerm.id],
        group: true,
      })
        .then((body) => {
          ...
          const ids = _.uniq(body.rows.map(row => row.value));
