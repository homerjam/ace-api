...
class File {
  ...
  search(query) {
    return new Promise((resolve, reject) => {
      ...
      Db.connect(this.config).searchAsync('file', 'all', query)
        .then(resolve, reject);
    });
  }
  ...
  create(file) {
    return new Promise((resolve, reject) => {
      ...
      Db.connect(this.config).insertAsync(file)
        .then(resolve, reject);
    });
  }
  ...
  delete(fileIds, slug) {
    return new Promise((resolve, reject) => {
      ...
      if (fileIds === 'trashed') {
        getFiles = Db.connect(this.config).viewAsync('file', 'trashed', {
          include_docs: true,
        });

      } else {
        getFiles = Db.connect(this.config).fetchAsync({
      ...
      getFiles.then((response) => {
        ...
        Promise.settle([assistPromise, s3Promise])
          .then((results) => {
            ...
            // Db.connect(this.config).viewAsync('file', 'revs', {
            //   keys: fileIds,
            // })
            //   .then((response) => {
            ...
            //     Db.connect(this.config).bulkAsync({ docs }).then(resolve, reject);
            //   }, reject);
